  ...
  while (Serial.available()) {
    ...
    switch (selected_command) {
      ...
      case 'v':
        ...
        Serial.print(FW_VER);
        break;
      case 'c': // Extra commands start from here
        command_cr_write(2000); //Literally no idea
      case 'b':
        command_cr_write(1000); //Literally no idea again
      case 's':
        command_cr_write(0);   //Literally no idea a third time
      default:
